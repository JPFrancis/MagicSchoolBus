...
public class ControlSystemLeft : MonoBehaviour {
	...
	// Use this for initialization
		...
		void Update () {
			...
			if (isPressed (SteamVR_Controller.DeviceRelation.Leftmost, SteamVR_Controller.ButtonMask.Trigger)) {
			...
			if(isPressed ( SteamVR_Controller.DeviceRelation.Leftmost, SteamVR_Controller.ButtonMask.Touchpad )){


				Vector2 PadButton = SteamVR_Controller.Input(SteamVR_Controller.GetDeviceIndex(SteamVR_Controller.DeviceRelation.Leftmost)).GetAxis();


				//UP
				if (PadButton.y > .4 && PadButton.x < .4) {
					print ("UP");
					cell.transform.position -= new Vector3 (.001F, 0, 0); 
